...
/**
 * A mediator to link a XTabLayout with a ViewPager2. The mediator will synchronize the ViewPager2's
 * position with the selected tab when a tab is selected, and the XTabLayout's scroll position when
 * the user drags the ViewPager2. XTabLayoutMediator will listen to ViewPager2's OnPageChangeCallback
 * to adjust tab when ViewPager2 moves. XTabLayoutMediator listens to XTabLayout's
 * OnTabSelectedListener to adjust VP2 when tab moves. XTabLayoutMediator listens to RecyclerView's
 ...
 * adapter and then call {@link #attach()} on it. Instantiating a XTabLayoutMediator will only create
 * the mediator object, {@link #attach()} will link the XTabLayout and the ViewPager2 together. When
 ...
 * {@link #attach()} call. Changing the ViewPager2 or XTabLayout will require a new instantiation of
 * XTabLayoutMediator.
 */
public final class TabLayoutMediator {
  @NonNull private final XTabLayout tabLayout;
  ...
  @Nullable private XTabLayoutOnPageChangeCallback onPageChangeCallback;
  @Nullable private XTabLayout.OnTabSelectedListener onTabSelectedListener;
  ...
  public interface TabConfigurationStrategy {
     ...
     * XTabLayout.Tab#setText(CharSequence)}, but any form of styling can be applied.
     *
  ...
  /**
   * Link the XTabLayout and the ViewPager2 together. Must be called after ViewPager2 has an adapter
   * set. To be called on a new instance of XTabLayoutMediator or if the ViewPager2's adapter
   * changes.
   *
  ...
  public void attach() {
    ...
    if (adapter == null) {
      throw new IllegalStateException(
          "XTabLayoutMediator attached before ViewPager2 has an " + "adapter");
    }
    attached = true;
  ...
  private static class XTabLayoutOnPageChangeCallback extends ViewPager2.OnPageChangeCallback {
    @NonNull private final WeakReference<XTabLayout> tabLayoutRef;
